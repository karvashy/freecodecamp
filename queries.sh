...
echo "$($PSQL "select distinct(name) from games right join teams on teams.team_id = games.opponent_id where year=2014 or winner_id is null order by name;")"
...
echo "$($PSQL "select name from games right join teams on games.winner_id=teams.team_id where name like 'Co%';")"
